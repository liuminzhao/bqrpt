...
C     postdensity
C------------------------------
      SUBROUTINE postdensity(grid, maxm, mdzero, nrec,
     &     alpha, mu, sigma2, v, whicho, whichn, like, ngrid)
      implicit none
      integer ngrid, i, nrec, maxm, mdzero
      real*8 grid(ngrid), alpha, mu, sigma2, v(nrec), like(ngrid)
      real*8 tmpll
      integer whicho(nrec), whichn(nrec)
