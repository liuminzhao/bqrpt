c===============================================
      subroutine heterptlmtri(maxm, mdzero, 
     &     nrec, nsub, p, q, x, y,
...
c     TUNING
      integer att1(p,q), att2(q), att3(6), att4, att5
      integer acc1(p,q), acc2(q), acc3(6), acc4, acc5
      real*8 tune1(p,q), tune2(q), tune3(6), tune4, tune5

      real*8 arate

      real*8 tunesave(nburn, p*q + q + 6+2)
      real*8 ratesave(nburn/50, p*q + q + 6+2)
      integer ratecount 

C     CURRENT
      real*8 beta(p, q), gamma(p, q), alpha, sigmavec(6), v(nrec)
...
C     INITIAL 
      ...
      sigmavec(3)=sqrt(Sigma(3,3))
      sigmavec(4)=Sigma(1,2)/sigmavec(1)/sigmavec(2)
      sigmavec(5)=Sigma(1,3)/sigmavec(1)/sigmavec(3)
      sigmavec(6)=Sigma(3,2)/sigmavec(3)/sigmavec(2)
...
C     log|Sigma|
      detlogl=log(Sigma(1,1)*Sigma(2,2)*Sigma(3,3) + 
     &     Sigma(2,1)*Sigma(3,2)*Sigma(1,3) + 
...
c     sigmavec(1)
         ...
         Sigmac(1,2)=exp(thetac)*sigmavec(2)*sigmavec(4)
         Sigmac(2,1)=exp(thetac)*sigmavec(2)*sigmavec(4)
         Sigmac(1,3)=exp(thetac)*sigmavec(3)*sigmavec(6)
         Sigmac(3,1)=exp(thetac)*sigmavec(3)*sigmavec(6)

c         detloglc=log(Sigmac(1,1)*Sigmac(2,2)-Sigmac(1,2)**2)
         detloglc=log(Sigmac(1,1)*Sigma(2,2)*Sigma(3,3) + 
     &        Sigmac(2,1)*Sigma(3,2)*Sigmac(1,3) + 
     &        Sigmac(1,2)*Sigma(2,3)*Sigmac(3,1) -
     &        Sigmac(1,3)*Sigma(2,2)*Sigmac(3,1) - 
     &        Sigmac(1,2)*Sigmac(2,1)*Sigma(3,3) - 
     &        Sigmac(1,1)*Sigma(2,3)*Sigma(3,2))
...
c     acceptance
     ...
     &        .lt. ratio) then 
            ...
            Sigma(1,3)=Sigmac(1,3)
            Sigma(3,1)=Sigmac(3,1)
            acc3(1)=acc3(1)+1
         end if
...
c     sigmavec(2)
         ...
         Sigmac(1,2)=exp(thetac)*sigmavec(1)*sigmavec(4)
         Sigmac(2,1)=exp(thetac)*sigmavec(1)*sigmavec(4)
         Sigmac(3,2)=exp(thetac)*sigmavec(3)*sigmavec(5)
         Sigmac(2,3)=exp(thetac)*sigmavec(3)*sigmavec(5)

c         detloglc=log(Sigmac(1,1)*Sigmac(2,2)-Sigmac(1,2)**2)
         detloglc=log(Sigma(1,1)*Sigmac(2,2)*Sigma(3,3) + 
     &        Sigmac(2,1)*Sigmac(3,2)*Sigma(1,3) + 
     &        Sigmac(1,2)*Sigmac(2,3)*Sigma(3,1) -
     &        Sigma(1,3)*Sigmac(2,2)*Sigma(3,1) - 
     &        Sigmac(1,2)*Sigmac(2,1)*Sigma(3,3) - 
     &        Sigma(1,1)*Sigmac(2,3)*Sigmac(3,2))
...
c     acceptance
     ...
     &        .lt. ratio) then 
            ...
            Sigma(1,2)=Sigmac(1,2)
            Sigma(2,1)=Sigmac(2,1)
            Sigma(3,2)=Sigmac(3,2)
            Sigma(2,3)=Sigmac(2,3)
            acc3(2)=acc3(2)+1
         end if

c     sigmavec(3)
         att3(3)=att3(3)+1
         theta=log(sigmavec(3))
         ...
         Sigmac(3,3)=exp(2*thetac)
         Sigmac(1,3)=exp(thetac)*sigmavec(1)*sigmavec(6)
         Sigmac(3,1)=exp(thetac)*sigmavec(1)*sigmavec(6)
         Sigmac(3,2)=exp(thetac)*sigmavec(2)*sigmavec(5)
         Sigmac(2,3)=exp(thetac)*sigmavec(2)*sigmavec(5)

         detloglc=log(Sigmac(1,1)*Sigmac(2,2)*Sigmac(3,3) + 
     &        Sigmac(2,1)*Sigmac(3,2)*Sigmac(1,3) + 
     &        Sigmac(1,2)*Sigmac(2,3)*Sigmac(3,1) -
     &        Sigmac(1,3)*Sigmac(2,2)*Sigmac(3,1) - 
     &        Sigmac(1,2)*Sigmac(2,1)*Sigmac(3,3) - 
     &        Sigmac(1,1)*Sigmac(2,3)*Sigmac(3,2))
...
c     acceptance
     ...
     &        .lt. ratio) then 
            loglikeo=loglikec
            detlogl=detloglc
            sigmavec(3)=exp(thetac)
            Sigma(3,3)=Sigmac(3,3)
            Sigma(1,3)=Sigmac(1,3)
            Sigma(3,1)=Sigmac(3,1)
            Sigma(3,2)=Sigmac(3,2)
            Sigma(2,3)=Sigmac(2,3)
...
c     sigmavec(4): rho1, rho12
         att3(4)=att3(4)+1
         theta=sigmavec(4)
         thetac=myrnorm(theta, tune3(4))
         thetac=max(thetac, -0.99)
         thetac=min(thetac, 0.99)

         if (sigmavec(5)**2 + sigmavec(6)**2 + thetac**2 <
     &        1 + 2*sigmavec(5)*sigmavec(6)*thetac) then
...
c     sigmavec(5): rho2, rho23
         att3(5)=att3(5)+1
         theta=sigmavec(5)
         thetac=myrnorm(theta, tune3(5))
         thetac=max(thetac, -0.99)
         thetac=min(thetac, 0.99)

         if (sigmavec(4)**2 + sigmavec(6)**2 + thetac**2 <
     &        1 + 2*sigmavec(4)*sigmavec(6)*thetac) then
            ...
            Sigmac(3,2)=thetac*sigmavec(3)*sigmavec(2)
            Sigmac(2,3)=thetac*sigmavec(3)*sigmavec(2)
...
c     sigmavec(6): rho3, rho13
         att3(6)=att3(6)+1
         theta=sigmavec(6)
         thetac=myrnorm(theta, tune3(6))
         thetac=max(thetac, -0.99)
         thetac=min(thetac, 0.99)

         if (sigmavec(4)**2 + sigmavec(5)**2 + thetac**2 <
     &        1 + 2*sigmavec(4)*sigmavec(5)*thetac) then
            ...
            Sigmac(1,3)=thetac*sigmavec(1)*sigmavec(3)
            Sigmac(3,1)=thetac*sigmavec(1)*sigmavec(3)
